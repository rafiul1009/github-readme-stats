...
**Shipping milestones**
- **MVP** = Phases 0–3 (streak + stats + top-langs + pins, working builder, copy-out)
- **v1.0** = Phases 0–7 (all Tier 1 + Tier 2 widgets, i18n, delivery modes)
- **v1.5+** = Phases 8–11
...
| 0.7 | Satori render pipeline | `src/lib/render/satori.ts`: JSX→SVG, font buffer loading, post-render injection of `<style>` keyframes and gradient `<defs>`. | P0 | todo |
| 0.8 | **Satori feasibility spike** | Time-boxed: prove the streak card's layout is reproducible in Satori's flexbox-only model *before* committing to it for chart widgets. Decide per-widget engine if not. | P0 | todo |
| 0.9 | Shared JSX primitives | `src/components/card/`: `Card`, `Row`, `Stat`, `Ring`, `Bar`, `Divider`, `Icon`, `Title` — all theme-slot driven. | P0 | todo |
| 0.10 | PNG output | `src/lib/render/png.ts` via `resvg-js`; animations auto-disabled for raster output. | P1 | todo |
...
| 0.16 | Reevaluate edge runtime | Current `runtime = 'edge'` on the SVG route conflicts with Satori font loading + resvg. Decide Node vs edge deliberately. | P1 | todo |
...
| # | Task | Details | Pri | Status |
| --- | --- | --- | --- | --- |
| 1.1 | Port streak card to Satori | Reproduce current layout: 3 columns, dividers, ring + fire icon, staged fade-ins. Visual-diff against current output. | P0 | todo |
...
| 2.7 | Top-langs: `donut`, `donut-vertical`, `pie` | Multi-segment ring/pie rendering (likely direct SVG rather than Satori — see 0.8). | P1 | todo |
...
## Phase 4 — Contribution graphs

| # | Task | Details | Pri | Status |
| --- | --- | --- | --- | --- |
| 4.1 | Activity graph widget | Contributions-over-time line/area chart; configurable date range, point/line/area style, grid toggle. | P0 | todo |
| 4.2 | Contribution heatmap | GitHub-style calendar grid; color-scale driven by theme slots. | P0 | todo |
| 4.3 | Graph axis/label system | Month/day labels, locale-aware, RTL-safe. | P1 | todo |
| 4.4 | Animated variants | Opt-in draw-on animation; disabled for PNG. | P2 | todo |
| 4.5 | Builder + mock data | | P0 | todo |
| 4.6 | Tests | Range boundaries, empty history, single-day history. | P1 | todo |

## Phase 5 — Trophies & profile summary family

| # | Task | Details | Pri | Status |
| --- | --- | --- | --- | --- |
| 5.1 | Trophy definitions | 6 standard (stars, commits, followers, issues, PRs, repos) + secret (multi-language, super-rank, new/ancient/long-time account, multi-org). Ranks `SSS SS S AAA AA A B C` + `UNKNOWN`/`SECRET`; documented thresholds. | P0 | todo |
| 5.2 | Trophy tile + next-rank bar | Title, rank, rank-title, value, progress-to-next-rank. | P0 | todo |
| 5.3 | Trophy grid options | `title=` filter, `rank=` filter/order, `column`, `row`, `margin-w`, `margin-h`, `no-bg`, `no-frame`. | P1 | todo |
| 5.4 | Profile summary card | Avatar (fetch, resize, base64-embed in `<pattern>`), name, followers/following, aggregate stats; `photo_quality`, `photo_resize`, `revert` layout flip, `{name}` title token. | P0 | todo |
| 5.5 | Repos-per-language card | | P1 | todo |
| 5.6 | Most-commit-language card | | P1 | todo |
| 5.7 | Productive-time card | Hour-of-day histogram + day-of-week activity. | P1 | todo |
| 5.8 | Builder + mock data | | P0 | todo |
| 5.9 | Tests | Tier boundaries, secret-trophy conditions, avatar fetch failure fallback. | P1 | todo |

## Phase 6 — Badges & icons

| # | Task | Details | Pri | Status |
| --- | --- | --- | --- | --- |
| 6.1 | Badge engine | `/api/widget/badges?name=a,b,c` composable rows; `column` (1–50), `size`, padding `p`, per-badge **theme cycling** from a comma list. | P0 | todo |
| 6.2 | User badge types | `visitors`, `repositories`, `followers`, `organization`, `languages`, `total-stars`, `total-contributors`, `total-commits`, `total-code-reviews`, `total-issues`, `total-pull-requests`, `total-joined-years`. | P0 | todo |
| 6.3 | Repo badge types | `stars`, `forks`, `contributors`, `issues`, `pull-requests`, `watchers`, `size`. | P1 | todo |
| 6.4 | Visitor counter | Persistent counter with **privacy-preserving IP hashing**; needs durable storage (ties to 10.1). | P1 | todo |
| 6.5 | Tech-icon grid | `name=` comma list, `columns` (1–50), index-mapped `color` list with fallback, `size`. | P1 | todo |
| 6.6 | Icon set + single-icon route | Bundled tech icon library; `/api/widget/icon/<name>`. | P1 | todo |
| 6.7 | Visual effects | `glow` and `wave` as a styling layer independent of theme. | P2 | todo |
| 6.8 | Builder support | Multi-select badge/icon pickers with live arrangement. | P1 | todo |

## Phase 7 — i18n & delivery modes → **v1.0**

| # | Task | Details | Pri | Status |
| --- | --- | --- | --- | --- |
| 7.1 | i18n infrastructure | `src/lib/i18n/`: label catalogs, locale-aware date + number formatting. | P0 | todo |
| 7.2 | 30 locale translations | GRS parity set. | P1 | todo |
| 7.3 | RTL layout mirroring | Mirror positions, not just strings. | P1 | todo |
| 7.4 | Custom `date_format` | Bracket syntax for conditional year (`M j[, Y]`), plus presets in the builder. | P2 | todo |
| 7.5 | GitHub Action | Renders the widget and commits the SVG to a user's profile repo — zero runtime dependency on our uptime. | P0 | todo |
| 7.6 | Builder emits workflow YAML | Copy-out tab producing a ready `.github/workflows/*.yml`. | P1 | todo |
| 7.7 | `format=json` everywhere | Short-circuit rendering; return raw stats. | P1 | todo |
| 7.8 | Self-host docs + Docker | Dockerfile, Vercel guide, `WHITELIST` env to restrict served usernames. | P1 | todo |
| 7.9 | Docs rewrite | Replace the stale [PROJECT-DOCUMENTATION.md](./PROJECT-DOCUMENTATION.md); auto-generate option tables from the Phase 0 schemas. | P0 | todo |
| 7.10 | Root README rewrite | Currently a stub; needs usage + embed examples per widget. | P0 | todo |
| 7.11 | Back-compat regression tests | Old streak URLs still render identically. | P0 | todo |

## Phase 8 — Profile README builder (Pillar B)

| # | Task | Details | Pri | Status |
| --- | --- | --- | --- | --- |
| 8.1 | Multi-widget composition | Pick several widgets, configure each, arrange them. | P0 | todo |
| 8.2 | Drag-to-reorder + layout | Side-by-side vs stacked; alignment; consistent widths across stacked cards. | P0 | todo |
| 8.3 | Shared theme lock | One theme applied across all selected widgets so a README looks coherent. | P0 | todo |
| 8.4 | Identity & socials section | Name, bio, social links → badge row generation. | P1 | todo |
| 8.5 | Tech-stack picker | Searchable icon/badge picker feeding the Phase 6 grid. | P1 | todo |
| 8.6 | Full README export | Emit a complete `README.md` with live rendered preview beside it. | P0 | todo |
| 8.7 | Starter templates | Minimal, Developer, Data Scientist, OSS Maintainer, Student. | P1 | todo |
| 8.8 | Save/share config | Encode the whole profile config in a shareable URL. | P2 | todo |
